...
<?php
 ...
 //Port Accept
   if ( isset($_POST['status']) && $_POST['status'] != ''&&($_POST['status']=="3")  && isset($_POST['port']) && $_POST['port'] != '') {
   	$port = $_POST['port'];
   	//echo"3";
	snmp2_set($ip230,$private,$oid3.".".$port,'i',"1");
       }
 //Port Suspend
         ...
         $port = $_POST['port'];
	 //echo"1or2";
	 snmp2_set($ip230,$private,$oid4.".".$port,'i',$status);
	 }
  ...
  //port ¿O«G©Î¨S«G
   echo" 
	 ...
	 <div class=\"row\">
	"; 
   $ifOperstatus = snmp2_real_walk("$ip230","$private","$oid5");
   $suspend_Operstatus =  snmp2_real_walk("$ip230","$private","$oid");
   ...
   foreach($ifOperstatus as $key => $value){
        ...
        $lab = "SELECT `st_num` ,`lab`  FROM `switch230` WHERE `port` = '$port_num[1]'";
